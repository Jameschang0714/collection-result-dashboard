...
@st.cache_data
def load_and_prep_data(attribution_path, group_path):
    try:
        ...
        group_df = pd.read_excel(
            ...
            header=None,
            usecols="A:C",
            names=[COL_GROUP, COL_AGENT_ID, COL_AGENT_NAME],
            engine='openpyxl'
        )
        ...
        st.error(f"錯誤：找不到指定的檔案。請檢查 `secrets.toml` 中的路徑是否正確。")
        st.error(f"檔案路徑: {e.filename}")
        return None
...
st.title("📊 電催績效追蹤歸因分析儀表板 V2.9")
st.caption("一個整合了時間序列分析與質效評估的決策支援平台")

try:
    ATTRIBUTION_PATH = st.secrets["ATTRIBUTION_PATH"]
    GROUP_LIST_PATH = st.secrets["GROUP_LIST_PATH"]
    df_raw = load_and_prep_data(ATTRIBUTION_PATH, GROUP_LIST_PATH)
except FileNotFoundError:
    st.error("`secrets.toml` 檔案未找到。請在專案根目錄下建立 `.streamlit/secrets.toml` 檔案並設定檔案路徑。")
    df_raw = None
except KeyError:
    st.error("請在 `.streamlit/secrets.toml` 中設定 `ATTRIBUTION_PATH` 和 `GROUP_LIST_PATH`。")
    df_raw = None
